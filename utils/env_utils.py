...
def encode_human_action(action):
    """
    把人类动作 [main_engine, side_thruster] 编码成离散整数 0..5
    main_engine: 0=off, 1=on
    side_thruster: 0=left, 1=neutral, 2=right
    code = main_engine * 3 + side_thruster
    """
    return int(action[0]) * 3 + int(action[1])